...
import {
  ...
  type YouPackage,
  type NetworkType,
} from "@/lib/youPackagesStore";

export const Route = createFileRoute("/admin/you-packages")({
  head: () => ({
    meta: [
      { title: "إدارة باقات شركة يو — الخليج تيليكوم" },
      { name: "robots", content: "noindex,nofollow" },
    ],
  }),
  component: AdminYouPackagesPage,
});

const emptyPackage = (): YouPackage => ({
  id: makeId("you-pkg"),
  name: "",
  price: "",
  internet: "—",
...
function AdminYouPackagesPage() {
  const navigate = useNavigate();
  const { categories, loading, refresh } = useYouPackagesStore();
    ...
    <div dir="rtl" className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        {loading && categories.length === 0 ? (
          ...
          <div className="rounded-xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground">
            لا توجد أقسام بعد. اضغط "إضافة قسم" لإنشاء أول قسم لباقات شركة يو.
          </div>
        ) : (
...
function PackageForm({
  ...
  initial: YouPackage;
  onCancel: () => void;
  onSave: (pkg: YouPackage) => void;
}) {
  const [form, setForm] = useState<YouPackage>(initial);
  const set = <K extends keyof YouPackage>(k: K, v: YouPackage[K]) =>
    setForm((f) => ({ ...f, [k]: v }));
