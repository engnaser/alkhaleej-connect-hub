...
import {
  ...
  type SabafonPackage,
  type NetworkType,
} from "@/lib/sabafonPackagesStore";

export const Route = createFileRoute("/admin/sabafon-packages")({
  head: () => ({
    meta: [
      { title: "إدارة باقات شركة سبافون — الخليج تيليكوم" },
      { name: "robots", content: "noindex,nofollow" },
    ],
  }),
  component: AdminSabafonPackagesPage,
});

const emptyPackage = (): SabafonPackage => ({
  id: makeId("you-pkg"),
  name: "",
  price: "",
  internet: "—",
...
function AdminSabafonPackagesPage() {
  const navigate = useNavigate();
  const { categories, loading, refresh } = useSabafonPackagesStore();
    ...
    <div dir="rtl" className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        {loading && categories.length === 0 ? (
          ...
          <div className="rounded-xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground">
            لا توجد أقسام بعد. اضغط "إضافة قسم" لإنشاء أول قسم لباقات شركة سبافون.
          </div>
        ) : (
                  ...
                  {editingCat === cat.id ? (
                    ...
                    <>
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <h2 className="text-lg font-extrabold text-foreground">{cat.title}</h2>
                          {cat.description && (
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button
                          onClick={() =>
                            guard(async () => {
                              if (!confirm("هل أنت متأكد من حذف هذا القسم وكل الباقات بداخله؟"))
                                return;
...
function CategoryForm({
  ...
  return (
    ...
    >
      {!inline && <h3 className="text-base font-extrabold text-primary">قسم جديد</h3>}
...
function PackageForm({
  ...
  initial: SabafonPackage;
  onCancel: () => void;
  onSave: (pkg: SabafonPackage) => void;
}) {
  const [form, setForm] = useState<SabafonPackage>(initial);
  const set = <K extends keyof SabafonPackage>(k: K, v: SabafonPackage[K]) =>
    setForm((f) => ({ ...f, [k]: v }));
    ...
    <div className="space-y-3 rounded-xl border border-primary/40 bg-card p-4 md:col-span-2">
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="اسم الباقة">
          <input value={form.name} onChange={(e) => set("name", e.target.value)} maxLength={100} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" />
        </Field>
        <Field label="السعر">
          <input value={form.price} onChange={(e) => set("price", e.target.value)} maxLength={50} placeholder="مثال: 500 ريال" className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" />
        </Field>
        <Field label="الإنترنت">
          <input value={form.internet} onChange={(e) => set("internet", e.target.value)} maxLength={50} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" />
        </Field>
        <Field label="الدقائق">
          <input value={form.minutes} onChange={(e) => set("minutes", e.target.value)} maxLength={50} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" />
        </Field>
        <Field label="الرسائل">
          <input value={form.sms} onChange={(e) => set("sms", e.target.value)} maxLength={50} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" />
        </Field>
        <Field label="الصلاحية">
          <input value={form.validity} onChange={(e) => set("validity", e.target.value)} maxLength={50} placeholder="مثال: 7 أيام" className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" />
        </Field>
        <Field label="نوع الشبكة">
          <select value={form.network} onChange={(e) => set("network", e.target.value as NetworkType)} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm">
        ...
        <Field label="كود التفعيل (اختياري)">
          <input value={form.code ?? ""} onChange={(e) => set("code", e.target.value)} maxLength={30} dir="ltr" placeholder="*500#" className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm font-mono" />
        </Field>
      </div>
      <div className="flex gap-2">
        <button onClick={submit} className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-xs font-bold text-primary-foreground">
          <Save className="h-3.5 w-3.5" />
          حفظ الباقة
        </button>
        <button onClick={onCancel} className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-background px-4 py-2 text-xs font-bold text-foreground">
          <X className="h-3.5 w-3.5" />
          إلغاء
        </button>
      </div>
